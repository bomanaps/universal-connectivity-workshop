#!/usr/bin/env python3
"""
Check script for Lesson 4: QUIC Transport (Nim)
Validates that the student's nim-libp2p solution can:
1. Establish a connection to the remote peer using QUIC
2. Send pings and receive pongs over QUIC
3. Display round-trip times (RTT)
"""
...
def check_output():
    ...
    try:
        ...
        # Check for ping with RTT - THIS IS CRITICAL
        ping_pattern = r"ping,(12D3KooW[A-Za-z0-9]+),(\d+\.?\d*)\s*ms"
        ping_matches = re.search(ping_pattern, output)
        if not ping_matches:
            print("x No ping with RTT received")
            print("i CRITICAL: The lesson must display round-trip time (RTT) when pings succeed!")
            print(f"i Actual output: {repr(output)}")
            return False
        ...
        print(f"v Ping received from {peerid_message} with RTT {ms} ms")
...
def main():
    """Main check function"""
    print("i Checking Lesson 4: QUIC Transport (Nim)")
    print("i " + "=" * 50)
    ...
    try:
        ...
        print("i " + "=" * 50)
        print("v QUIC Transport completed successfully!")
        print("i You have successfully:")
        print("i - Configured QUIC transport")
        print("i - Established bidirectional QUIC connectivity")
        print("i - Measured and displayed round-trip times (RTT)")
        print("i - Demonstrated transport abstraction in libp2p")
        print("i Ready for Lesson 5: Identify Checkpoint!")
