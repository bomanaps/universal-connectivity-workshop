#!/usr/bin/env python3
"""
Check script for Lesson 7: Kademlia Checkpoint (Nim)
Validates that the student's nim-libp2p solution can:
1. Establish a connection to the remote peer
2. Initialize and use Kademlia DHT
3. Store and retrieve values in the DHT
"""
...
def check_output():
    ...
    try:
        ...
        print("i Checking Kademlia DHT checkpoint functionality...")
        ...
        # Check for Kademlia DHT initialization
        kad_pattern = r"kademlia,(12D3KooW[A-Za-z0-9]+)"
        kad_matches = re.search(kad_pattern, output)
        if not kad_matches:
            print("x No Kademlia DHT initialization detected")
            print("i CRITICAL: The peer must initialize Kademlia DHT!")
            print(f"i Actual output: {repr(output)}")
            return False

        peerid = kad_matches.group(1)
        print(f"v Kademlia DHT initialized for peer {peerid[:12]}...")

        # Check for DHT operations (put/get) - optional but good to have
        dht_op_pattern = r"dht-(put|get),(12D3KooW[A-Za-z0-9]+)"
        dht_op_matches = re.search(dht_op_pattern, output)
        if dht_op_matches:
            op = dht_op_matches.group(1)
            peerid = dht_op_matches.group(2)
            print(f"v DHT {op} operation detected for peer {peerid[:12]}...")
...
def main():
    """Main check function"""
    print("i Checking Lesson 7: Kademlia Checkpoint (Nim)")
    print("i " + "=" * 50)
    ...
    try:
        ...
        print("i " + "=" * 50)
        print("v Kademlia DHT checkpoint completed successfully!")
        print("i You have successfully:")
        print("i - Established a connection with the remote peer")
        print("i - Initialized Kademlia DHT")
        print("i - Participated in DHT operations")
        print("i - Reached your fourth checkpoint!")
        print("i Ready for the final lesson!")
