#!/usr/bin/env python3
"""
Check script for Lesson 5: Identify Checkpoint (Nim)
...
3. Send pings and receive pongs with RTT
"""
...
def check_output():
    ...
    try:
        ...
        # Check for identify exchange
        identify_pattern = r"identify,(12D3KooW[A-Za-z0-9]+)"
        identify_matches = re.search(identify_pattern, output)
        if not identify_matches:
            print("x No identify exchange detected")
            print("i CRITICAL: The Identify protocol must be exchanged!")
            print(f"i Actual output: {repr(output)}")
            return False
        ...
        print(f"v Identify exchanged with {peerid_message}")

        # Check for ping with RTT
...
def main():
    """Main check function"""
    print("i Checking Lesson 5: Identify Checkpoint (Nim)")
    print("i " + "=" * 50)
